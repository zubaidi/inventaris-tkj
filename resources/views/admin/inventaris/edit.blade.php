                ...
                <form action="{{ route('admin.inventaris.update', $barang->id) }}" method="POST">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', $barang->tanggal?->format('Y-m-d')) }}" required>
                            @error('tanggal')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="col-md-6">
                            <label for="no_inventaris" class="form-label">No Inventaris <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('no_inventaris') is-invalid @enderror" id="no_inventaris" name="no_inventaris" value="{{ old('no_inventaris', $barang->no_inventaris) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('tahun_pembelian') is-invalid @enderror" id="tahun_pembelian" name="tahun_pembelian" value="{{ old('tahun_pembelian', $barang->tahun_pembelian) }}" min="2000" max="{{ date('Y') }}" required>
                            @error('tahun_pembelian')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="col-md-12">
                            <label for="spesifikasi" class="form-label">Spesifikasi</label>
                            <textarea class="form-control @error('spesifikasi') is-invalid @enderror" id="spesifikasi" name="spesifikasi" rows="3">{{ old('spesifikasi', $barang->spesifikasi) }}</textarea>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="number" class="form-control @error('volume') is-invalid @enderror" id="volume" name="volume" value="{{ old('volume', $barang->volume) }}" min="1" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('lab_id') is-invalid @enderror" id="lab_id" name="lab_id" required>
                                ...
                                @foreach ($labs as $lab)
                                    <option value="{{ $lab->id }}" {{ old('lab_id', $barang->lab_id) == $lab->id ? 'selected' : '' }}>{{ $lab->nama_lab }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('sumber_dana_id') is-invalid @enderror" id="sumber_dana_id" name="sumber_dana_id" required>
                                ...
                                @foreach ($sumberDanas as $sumberDana)
                                    <option value="{{ $sumberDana->id }}" {{ old('sumber_dana_id', $barang->sumber_dana_id) == $sumberDana->id ? 'selected' : '' }}>{{ $sumberDana->nama }}</option>
                                @endforeach
                            </select>
